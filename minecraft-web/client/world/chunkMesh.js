...
export function buildChunkMesh(chunk, material, textures, sampleGlobal, sampleLight) {
      ...
      for (let j = 0; j < dims[v]; j += 1) {
        for (let i = 0; i < dims[u]; ) {
          ...
          for (const c of corners) {
            ...
            const light = Math.max(0.08, sampleLight(c[0] + chunk.cx * CHUNK_SIZE, c[1], c[2] + chunk.cz * CHUNK_SIZE) / 15); // Phase2
  ...
  geometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3)); // Phase2
