...
export function serializeChunk(chunk) {
  ...
  const out = Buffer.allocUnsafe(source.length * 2 + 2);
  out.writeUInt16LE(CHUNK_VERSION, 0);
  for (let i = 0; i < source.length; i += 1) {
    out.writeUInt16LE(source[i], i * 2 + 2);
  }
  return out;
}
...
export function deserializeChunk(buffer) {
  ...
  if (view.length === BLOCK_COUNT * 2) {
    const legacy = createChunkBuffer();
    for (let i = 0; i < legacy.length; i += 1) legacy[i] = view.readUInt16LE(i * 2);
    return legacy;
  }

  if (view.length !== BLOCK_COUNT * 2 + 2) {
    throw new Error(`Invalid chunk byte length: expected ${BLOCK_COUNT * 2 + 2}, received ${view.length}`);
  }

  const version = view.readUInt16LE(0);
  if (version !== CHUNK_VERSION) throw new Error(`Unsupported chunk version ${version}`);
  const chunk = createChunkBuffer();
  for (let i = 0; i < chunk.length; i += 1) {
    chunk[i] = view.readUInt16LE(i * 2 + 2);
  }
  return chunk;
}

export { CHUNK_SIZE, CHUNK_HEIGHT, BLOCK_COUNT, CHUNK_VERSION };
